...
class CocoSequenceGenerator(SequenceGenerator):
  def __init__(self, coco, batch_num_streams, image_root, vocab=None,
    ...
    if align:
      ...
      #  print 'after num_pairs {}'.format(len(self.image_sentence_pairs))
      #  for sentence in self.image_sentence_pairs:
  ...
  def next_line(self):
    ...
    print 'next line current index {}'.format(self.index)
    self.index += 1
  ...
  def line_to_stream(self, sentence):
    ...
    assert(len(self.vocabulary) + 1 not in stream)
    #  print 'convert sentence {} to {}'.format(sentence, stream)
    return stream

  def get_pad_value(self, stream_name):
    if len(self.vocabulary) == 0:
      raise Exception("init vocabulary")
    return len(self.vocabulary) + 1 if stream_name in self.negative_one_padded_streams else 0
  ...
  def get_streams(self):
    ...
    pad_value = len(self.vocabulary) + 1
    self.num_outs += 1
    out = {}
    out['stage_indicators'] = [1] * (len(stream) + 1) + [0] * pad
    out['cont_sentence'] = [0] + [1] * len(stream) + [0] * pad
    out['input_sentence'] = [0] + stream + [pad_value] * pad
    assert(-1 not in out['input_sentence'])
    #  print 'pad sentence {} to {}'.format(stream, out['input_sentence'])
    out['target_sentence'] = stream + [0] + [pad_value] * pad
    truncated = False
    if self.truncate:
