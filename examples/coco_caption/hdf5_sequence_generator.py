...
class SequenceGenerator():
  ...
  def reset_stream(self, stream_index):
    #  print 'reset stream index {}'.format(stream_index)
    streams = self.get_streams()
    #  print 'reset_stream input_sentence {}'.format(streams['input_sentence'])
  ...
  def get_next_batch(self, truncate_at_exhaustion=True):
    ...
    for t in range(self.batch_stream_length):
      ...
      for i in range(self.batch_num_streams):
        #  print 'excute timestep {} and batch_num_streams {}'.format(t, i)
        if not exhausted[i]:
          ...
          for name in self.substream_names:
            ...
            #  print ("set batch {},{} to {}".format(t, i, batch[name][t, i]))
            #  if name == 'input_sentence':
                #  print ("set batch({}) {},{} to {}".format(name, t, i, batch[name][t, i]))
      ...
      if all_exhausted and truncate_at_exhaustion:
        ...
        break
    print 'batch input_sentence {}'.format(batch['input_sentence'].shape)
