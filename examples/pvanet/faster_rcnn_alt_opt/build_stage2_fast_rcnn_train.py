...
def make_fully(n, name, num_output, last_layer, use_global_stats):
    n[name] = ip = L.InnerProduct(last_layer, name=name, param=[dict(lr_mult=1, decay_mult=1.0), dict(lr_mult=2.0, decay_mult=0)], inner_product_param=dict(num_output=num_output, weight_filler = dict(type = 'xavier'), bias_filler = dict(type = 'constant', value = 0.1)))
    #  # batch normalization
