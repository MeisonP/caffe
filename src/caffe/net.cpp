...
template <typename Dtype>
Net<Dtype>::Net(const NetParameter& param, shared_ptr<Net<Dtype> > shared_net) {
  Init(param, shared_net);
}
...
template <typename Dtype>
void Net<Dtype>::Init(const NetParameter& in_param, shared_ptr<Net<Dtype> > shared_net) {
...
inline string create_or_link(boost::unordered_map<string, string>& record, const string& blob_name, const string& suffix){
...
inline void exclude_both(boost::unordered_map<string, string>& record, std::set<string>& ex_set, const string& blob_name){
