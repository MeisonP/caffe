...
/* __global__ void CopyToTop(Dtype* tmp1, Dtype *tmp2, Dtype *t1, Dtype *t2, int count, Dtype* boundary_data){ */
/* } */
...
__global__ void SelectBoxAftNMS(const int nthreads, const float *in_box, int *keep_indices,
                          Dtype *top_data, const Dtype *in_score, Dtype* top_score, int batch_ind) {
  CUDA_KERNEL_LOOP(index , nthreads) {
    top_data[index * 5] = batch_ind;
...
void FrcnnProposalLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
  ...
  Dtype bottom_im_info[6];
  CHECK_EQ(bottom[2]->count(), 6);
  ...
  std::shared_ptr<Blob<Dtype> > tmp_rois(new Blob<Dtype>());
  std::shared_ptr<Blob<Dtype> > tmp_scores(new Blob<Dtype>());
  /* Blob<Dtype> tmp_rois; */
  /* Blob<Dtype> tmp_scores; */
  tmp_rois->Reshape(num * rpn_post_nms_top_n, 5, 1, 1);
  tmp_scores->Reshape(num * rpn_post_nms_top_n, 1, 1, 1);

  Dtype * tmp_rois_data = tmp_rois->mutable_gpu_data();
  Dtype * tmp_scores_data = tmp_scores->mutable_gpu_data();

  /* top[0]->Reshape(num * rpn_post_nms_top_n, 5, 1, 1); */
  /* top[1]->Reshape(num * rpn_post_nms_top_n, 1, 1, 1); */
  top[2]->Reshape(num, 1, 1, 1);
  Dtype *boundary_data = top[2]->mutable_cpu_data();

  /* Dtype *top_data = top[0]->mutable_gpu_data(); */
  /* Dtype *top_score = top[1]->mutable_gpu_data(); */
  ...
  Dtype *fg_scores = (Dtype*)(&bottom_rpn_score[(n * (2 * config_n_anchors) * height * width) + total_anchor_num]);
  ...
  BBoxTransformInv<Dtype><<<caffe::CAFFE_GET_BLOCKS(retained_anchor_num), caffe::CAFFE_CUDA_NUM_THREADS>>>(
      retained_anchor_num, bottom_rpn_bbox + (n * config_n_anchors * 4 * height * width), height, width, FrcnnParam::feat_stride,
  ...
  CUDA_CHECK(cudaMalloc(&bbox_score_, sizeof(Dtype) * retained_anchor_num));
  ...
  int keep_num = -1;
  gpu_nms(&keep_indices[0], &keep_num, transform_bbox_, selected_num, 4, rpn_nms_thresh, false);
  ...
  SelectBoxAftNMS<<<caffe::CAFFE_GET_BLOCKS(keep_num), caffe::CAFFE_CUDA_NUM_THREADS>>>(
      keep_num, transform_bbox_, gpu_keep_indices_, tmp_rois_data + (n * rpn_post_nms_top_n * 5), bbox_score_, tmp_scores_data + (n * rpn_post_nms_top_n * 1), n);
  boundary_data[n] = keep_num;
  count += keep_num;
  ...
  // copy tmp to top
  top[0]->Reshape(count, 5, 1, 1);
  top[1]->Reshape(count, 1, 1, 1);
  if (count == rpn_post_nms_top_n * num) {
    top[0]->CopyFrom(*tmp_rois);
    top[1]->CopyFrom(*tmp_scores);
  } else {
    Dtype* bbox_data = top[0]->mutable_cpu_data();
    Dtype* scores_data = top[1]->mutable_cpu_data();
    Dtype* tmp_bbox_data = tmp_rois->mutable_cpu_data();
    Dtype* tmp_scores_data = tmp_scores->mutable_cpu_data();
    int a = 0;
    for (int n = 0; n < num; n++) {
      for (size_t i = 0; i < boundary_data[n]; i++) {
        int batch_ind = tmp_bbox_data[n * rpn_post_nms_top_n * 5 + i * 5];
        CHECK(batch_ind > 0);
        CHECK(batch_ind < num);
        bbox_data[a * 5 + i * 5] = batch_ind;
        for (int j = 1; j < 5; j++) {
          bbox_data[a * 5 + i * 5 + j] = tmp_bbox_data[n * rpn_post_nms_top_n * 5 + i * 5 + j];
        }
        scores_data[a + i] = tmp_scores_data[n * rpn_post_nms_top_n * 1 + i];
      }
      a += boundary_data[n];
    }
  }
  ...
  LOG(INFO) << "Proposal layer Forward takes: "<<  t1.MilliSeconds() / num;
