...
double get_scale_factor(int width, int height, int short_size, int max_long_size) {
  int im_size_min = std::min(width, height);
  int im_size_max = std::max(width, height);

  double scale_factor = static_cast<double>(short_size) / im_size_min;
  // Prevent the biggest axis from being more than max_size
  if (scale_factor * im_size_max > max_long_size) {
    scale_factor = static_cast<double>(max_long_size) / im_size_max;
  }
