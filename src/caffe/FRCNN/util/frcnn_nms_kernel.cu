...
void gpu_nms(int* keep_out, int* num_out, float* boxes_host_or_dev, int boxes_num,
          int boxes_dim, float nms_overlap_thresh, bool copy_boxes_to_dev, int device_id) {
  ...
  if (!copy_boxes_to_dev) {
    boxes_dev = boxes_host_or_dev;
  } else {
