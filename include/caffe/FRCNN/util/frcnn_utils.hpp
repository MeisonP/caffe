...
public:
  ...
  string to_string() const {
    char buff[100];
    snprintf(buff, sizeof(buff), "%.10f %.10f %.10f %.10f", Point[0], Point[1], Point[2], Point[3]);
    return string(buff);
  }
...
public:
  ...
  float area() {
    return (this->Point[2] - this->Point[0] + 1) * (this->Point[3] - this->Point[1] + 1);
  }

  inline string to_string() const {
    char buff[100];
    snprintf(buff, sizeof(buff), "cls:%3d -- (%.3f): %.10f %.10f %.10f %.10f", id,
...
double get_scale_factor(int width, int height, int short_size, int max_long_size);
...
// Vector<float> generate_anchors(Vector<float> );
