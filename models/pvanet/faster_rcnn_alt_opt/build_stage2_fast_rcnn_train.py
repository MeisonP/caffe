...
def build_residual_c_rulu_block(n, stage, block, use_global_stats, num_output, last_layer, residual_mode='conv', with_batch_normalization_on_left=False, replace_last_layer=False, to_seperable=False, new_depth=False, zero=False):
    ...
    c1 = build_c_relu_block_1(n, stage, block, last_layer, use_global_stats, bn_inplace=True, num_output=num_output[0], with_bn=with_batch_normalization_on_left, new_depth=new_depth, zero=zero)
    c2 = build_c_relu_block_2(n, stage, block, c1, use_global_stats, bn_inplace=False,  num_output=num_output[1], zero=zero)
    c3 = build_c_relu_block_3(n, stage, block, c2, use_global_stats, bn_inplace=False,  num_output=num_output[2], zero=zero)
...
def build_inception_block(n, stage, block, last_layer, use_global_stats, module_params, pool_param, residual_mode='conv', num_output=256, last=False, to_seperable=False, new_depth=False, zero=False):
    ...
    incep_0 = build_inception_module(n, stage, block, 'incep', '0', inception_param_0, use_global_stats, relu, zero=zero)
    # second module
    inception_param_reduce_1 = module_params['incep_reduce_1']
    reduce_out = build_inception_module(n, stage, block, 'incep', '1_reduce', inception_param_reduce_1, use_global_stats, relu, zero=zero)
    inception_param_1_0 = module_params['incep_1_0']
    incep_1_0 = build_inception_module(n, stage, block, 'incep', '1_0', inception_param_1_0, use_global_stats, reduce_out, to_seperable=to_seperable, last_layer_out=module_params['incep_reduce_1']['num_output'], zero=zero)
    # third module
    inception_param_reduce_2 = module_params['incep_reduce_2']
    reduce_out = build_inception_module(n, stage, block, 'incep', '2_reduce', inception_param_reduce_2, use_global_stats, relu, to_seperable=to_seperable, zero=zero)
    inception_param_2_0 = module_params['incep_2_0']
    incep_2_0 = build_inception_module(n, stage, block, 'incep', '2_0', inception_param_2_0, use_global_stats, reduce_out, to_seperable=to_seperable, last_layer_out=module_params['incep_reduce_2']['num_output'], zero=zero)
    inception_param_2_1 = module_params['incep_2_1']
    incep_2_1 = build_inception_module(n, stage, block, 'incep', '2_1', inception_param_2_1, use_global_stats, incep_2_0, to_seperable=to_seperable, last_layer_out=module_params['incep_2_1']['num_output'], zero=zero)
    ...
    if bool(pool_param):
        ...
        n['conv{0}_{1}/incep/poolproj'.format(stage, block)] = poolproj = build_inception_module(n, stage, block, 'incep', 'poolproj', inception_param_poolproj, use_global_stats, incep_pool, zero=zero)
...
def make_fully(n, name, num_output, last_layer, use_global_stats):
    n[name] = ip = L.InnerProduct(last_layer, name=name, param=[dict(name='{}_w'.format(name), lr_mult=1.0, decay_mult=1.0), dict(name = '{}_b'.format(name), lr_mult=2.0, decay_mult=0)], inner_product_param=dict(num_output=num_output, weight_filler = dict(type = 'xavier'), bias_filler = dict(type = 'constant', value = 0.1)))
    #  # batch normalization
    n['{}/bn'.format(name)] = bn = L.BatchNorm(ip, param=[dict(name='{}_mean'.format(name), lr_mult=0),dict(name='{}_var'.format(name), lr_mult=0),dict(name='{}_mov'.format(name), lr_mult=0)], in_place=True, use_global_stats=use_global_stats)
    #scale
    scale_param=[dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0)]
    n['{}/scale'.format(name)] = scale = L.Scale(bn, scale_param=dict(bias_term=True), in_place=True, param=[dict(name='{}_alpha'.format(name), lr_mult=1, decay_mult=0), dict(name='{}_beta'.format(name), lr_mult=1, decay_mult=0)])
    # dropout
...
def make_fully_readonly(n, name, num_output, last_layer, use_global_stats):
    orig = name
    name = name[0:3]
    n[name + '_readonly'] = ip = L.InnerProduct(last_layer, name=orig, param=[dict(name='{}_w'.format(name), lr_mult=1.0, decay_mult=1.0), dict(name = '{}_b'.format(name), lr_mult=2.0, decay_mult=0)], inner_product_param=dict(num_output=num_output, weight_filler = dict(type = 'xavier'), bias_filler = dict(type = 'constant', value = 0.1)), propagate_down=0)
    #  # batch normalization
    n['{}/bn_readonly'.format(name)] = bn = L.BatchNorm(ip, param=[dict(name='{}_mean'.format(name), lr_mult=0),dict(name='{}_var'.format(name), lr_mult=0),dict(name='{}_mov'.format(name), lr_mult=0)], in_place=True, use_global_stats=use_global_stats, propagate_down=0)
    #scale
    scale_param=[dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0)]
    n['{}/scale_readonly'.format(name)] = scale = L.Scale(bn, scale_param=dict(bias_term=True), in_place=True, param=[dict(name='{}_alpha'.format(name), lr_mult=1, decay_mult=0), dict(name='{}_beta'.format(name), lr_mult=1, decay_mult=0)], propagate_down=0)
    # dropout
    n['{}/dropout_readonly'.format(name)] = dropout = L.Dropout(scale, in_place=True, dropout_param=dict(dropout_ratio=0.5), propagate_down=0)
    # relu
    n['{}/relu_readonly'.format(name)] = relu = L.ReLU(dropout, in_place=True, propagate_down=0)
...
def write_prototxt(is_train, source, output_folder, to_seperable=False):
    ...
    # concat 
    netspec['convf'] = convf = L.Concat(reluf_rpn, reluf2, concat_param=dict(axis=1), name='concat_convf')
    ...
    # OHEM
    n.roi_pool5_readonly = L.ROIPooling(convf, n.rois, roi_pooling_param=dict(pooled_w=6, pooled_h=6, spatial_scale=0.0625), propagate_down=[0,0])
    fc6_readonly = make_fully_readonly(netspec, 'fc6_readonly', 4096, n.roi_pool5_readonly, use_global_stats)
    fc7_readonly = make_fully_readonly(netspec, 'fc7_readonly', 4096, fc6_readonly, use_global_stats)
    n.cls_score_readonly = L.InnerProduct(fc7_readonly, propagate_down=0, param=[dict(name='cls_score_w'), dict(name='cls_score_b')], num_output=cls_num)
    n.bbox_pred_readonly = L.InnerProduct(fc7_readonly, propagate_down=0, param=[dict(name='bbox_pred_w'), dict(name='bbox_pred_b')], num_output=cls_num * 4)
    n.cls_prob_readonly = L.Softmax(n.cls_score_readonly, propagate_down=0)
    n.rois_hard, n.labels_hard, n.bbox_targets_hard, n.bbox_inside_weights_hard, n.bbox_outside_weights_hard = \
            L.Python(n.cls_prob_readonly, n.bbox_pred_readonly, n.rois, n.labels, n.bbox_targets, n.bbox_inside_weights, n.bbox_outside_weights, propagate_down=[0,0,0,0,0,0,0], ntop = 5, python_param = dict(module='caffe.frcnn.ohem_layer.layer', layer = 'OHEMDataLayer', param_str='num_classes: 6'))


    n.roi_pool_conv5 = L.ROIPooling(convf, n.rois_hard, roi_pooling_param=dict(pooled_w=6, pooled_h=6, spatial_scale=0.0625))
    ...
    n.loss_cls = L.SoftmaxWithLoss(n.cls_score, n.labels_hard, propagate_down=[1,0], loss_weight=1, loss_param=dict(ignore_label=1, normalize=True))
    n.loss_bbox = L.SmoothL1Loss(n.bbox_pred, n.bbox_targets_hard, n.bbox_inside_weights_hard, n.bbox_outside_weights_hard, loss_weight=1)
...
if __name__ == '__main__':
    ...
    filepath = './stage2_fast_rcnn_train.prototxt'
