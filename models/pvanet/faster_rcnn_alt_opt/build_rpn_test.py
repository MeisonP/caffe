...
def write_prototxt(is_train, source, output_folder, to_seperable=False):
    ...
    netspec.data = L.Input(input_param=dict(shape=dict(dim=[1,3,640,1056])))

    netspec.im_info = L.Input(input_param=dict(shape=dict(dim=[1,6])))
    # stage1
    # C.RELU block
    ...
    n.rpn_cls_prob = L.Softmax(rpn_cls_score_reshape)
    n.rpn_cls_prob_reshape = L.Reshape(n.rpn_cls_prob, reshape_param=dict(shape=dict(dim=[0, 2 * anchor_num, -1, 0])))

    n.rois, n.scores = \
            L.Python(n.rpn_cls_prob_reshape, rpn_bbox_pred, n.im_info, name='proposal', python_param=dict(module='caffe.frcnn.rpn.proposal_layer', layer='ProposalLayer', param_str="{'feat_stride': 16, 'ratios': [0.5, 0.667, 1.0, 1.5, 2.0], 'scales': [3, 6, 9, 16, 25]}"), ntop=2)
